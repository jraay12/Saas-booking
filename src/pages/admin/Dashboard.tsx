...
import { useGetDashboard } from "../../features/dashboard/dashboard.hook";
      ...
      <div className="mx-auto space-y-6">
          ...
          <div className="hidden sm:flex items-center gap-2.5 bg-indigo-50 border border-indigo-100 rounded-2xl px-4 py-3">
            ...
            <div>
              ...
              <p className="text-[13px] font-semibold text-indigo-900">
                {dashboard?.TODAY_SCHEDULE?.length ?? 0} appointments
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
          <StatCard
            ...
            value={dashboard?.TODAY_SCHEDULE.length ?? 0}
            sub="Scheduled today"
            accentFrom="#534AB7"
            accentTo="#7F77DD"
            iconBg="bg-indigo-50"
          ...
          <StatCard
            label="Confirmed"
            value={dashboard?.stats?.totalConfirmedBookings ?? 0}
            sub="Ready to go"
            accentFrom="#185FA5"
            accentTo="#378ADD"
            iconBg="bg-blue-50"
          ...
          <StatCard
            label="Pending"
            value={dashboard?.stats?.totalPendingBookings ?? 0}
          ...
          <StatCard
            label="Revenue"
            value={`₱${(dashboard?.stats?.revenue ?? 0).toLocaleString("en-PH")}`}
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
          <div className="xl:col-span-2">
            <RevenueChart REVENUE_DATA={dashboard?.REVENUE_DATA} />
          </div>
          <div>
            <StatusDonutChart STATUS_DONUT={dashboard?.STATUS_DONUT} />
          </div>
        </div>

        {/* ── Bottom Row ── */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <TodaySchedule TODAY_SCHEDULE={dashboard?.TODAY_SCHEDULE} />
          <TopServices />
        </div>
      </div>
    </div>
