...
import {
  ...
  removeAssignStaff,
  getAllServicePublic,
  getAssignedStaffInServicePublic,
  getServiceById,
} from "./service.api";
...
export const useGetAllAssignedStaffPublic = (id: string) => {
  return useQuery({
    queryKey: queryKeys.assignedStaff(businessId!, id),
    queryFn: () => getAssignedStaffInServicePublic(id),
    enabled: !!id,
  });
};
...
export const useGetServiceById = (id: string) => {
  return useQuery({
    ...
    queryKey: queryKeys.service(businessId!, id),
  });
};
